...
int RA8875::KeypadInit(bool scanEnable, bool longDetect, uint8_t sampleTime, uint8_t scanFrequency,
                             uint8_t longTimeAdjustment, bool interruptEnable, bool wakeupEnable)
{
    uint8_t value = 0;

    pKeyMap = DefaultKeyMap;
...
bool RA8875::KeypadHit(void)
{
    uint8_t value = 0;
    writeCommand(0xF1);
    value= _readData(false);
    return ((value & 0x10) == 0x10);  // check KS status - true if kbhit
}

void RA8875::SetKeyMap(const uint8_t * CodeList)
{
    pKeyMap = CodeList;
}

uint8_t RA8875::KeypadGetKey(void)
{
    uint8_t keyNumReg;
    uint8_t keyCode3;
    uint8_t keyCode2;
    ...
    writeCommand(0xC1);
    keyNumReg=_readData(false);
    keyNumReg=keyNumReg & 0x03;
    switch(keyNumReg) {
        case 0x01:
            writeCommand(0xC2);
            keyCode3=_readData(false);
            keyCode2=0;
            keyCode1=0;
            break;
        case 0x02:
