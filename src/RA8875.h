...
15/6/2023 Added KeyScan inteface - John Melton G0ORX
...
// KEYPAD
     ...
     int KeypadInit(bool scanEnable, bool longDetect, uint8_t sampleTime, uint8_t scanFrequency,
                             uint8_t longTimeAdjustment, bool interruptEnable, bool wakeupEnable);

     bool KeypadHit(void);
     uint8_t KeypadGetKey(void);

     void SetKeyMap(const uint8_t * CodeList = NULL);
